...
if (!function_exists('ag_render_content_header')) {
    ...
    function ag_render_content_header(array $config): void
    {
        ...
        if ($showNavigation && !empty($appNavigation)) {
            $configurationRoutes = [
                'clientesConfiguracion',
                'contratosConfiguracion',
                'solicitudesConfiguracion',
                'parametros',
                'roles',
                'consola',
            ];

            $filterNavigationItems = static function (array $navigation) use ($configurationRoutes): array {
                $filtered = [];
                ...
                foreach ($navigation as $entry) {
                    ...
                    $entryRoute = '';
                    if ($entryUrl !== '' && preg_match('/ruta=([^&#]+)/', $entryUrl, $match)) {
                        $entryRoute = rawurldecode((string)($match[1] ?? ''));
                    }

                    if ($entryRoute !== '' && in_array($entryRoute, $configurationRoutes, true)) {
                        continue;
                    }

                    $children = [];
                    $rawChildren = isset($entry['children']) && is_array($entry['children']) ? $entry['children'] : [];
                    foreach ($rawChildren as $child) {
                        ...
                        $childRoute = '';
                        if ($childUrl !== '' && preg_match('/ruta=([^&#]+)/', $childUrl, $childMatch)) {
                            $childRoute = rawurldecode((string)($childMatch[1] ?? ''));
                        }

                        if ($childRoute !== '' && in_array($childRoute, $configurationRoutes, true)) {
                            continue;
                        }
            ...
            $filteredNavigation = $filterNavigationItems($appNavigation);
